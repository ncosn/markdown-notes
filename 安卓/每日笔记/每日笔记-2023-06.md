...
Looper.prepare会`new Looper()`并且挂载到ThreadLocal上，于是当前线程绑定了创建的Looper对象；而Looper.myLooper就是根据当前线程拿到创建的Looper；
...
>
> - 装饰模式是强调**对原有类型自身能力的拓展**；代理模式强调**控制对象的访问**；
...
> 
>
> **装饰者模式的思考过程**：我现在有一个业务类，但因为业务不同，有时候需要扩展A功能，有时候需要扩展B功能，所以采用装饰者模式进行增强。
>
> **代理模式的思考过程**：我现在有A、B、C三种类型的客户，他们都要访问一个功能对象，但功能对象应该根据客户类型进行一些改动和增强，这个时候就使用代理模式。

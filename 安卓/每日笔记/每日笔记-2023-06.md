...
+ **@Retention**-什么时候使用该注解
  RetentionPolicy.SOURCE（默认）：源文件保留（ide使用，编译完就没了）
  RetentionPolicy.CLASS：class保留（butterknife，ARoute）
...
### Handler（[尚硅谷](https://www.bilibili.com/video/BV1VE411Z7Ay/?spm_id_from=333.337.search-card.all.click&vd_source=bf286743c0f79ff4e4df2222645065d3)）
...
Handler {//发送消息，处理消息，移除消息
    ...
    handlerMessge(Message msg) //处理消息的回调方法（一般在主线程中重写）
    ...
    public void dispatchMessage(Message msg) {
        if(msg.callback != null) {//如果消息自己可以处理，让消息自己处理
            ...
            if(mCallback != null) {//如果handler对象中有回调监听器，则调用回调器来处理消息
...
// 2发送粘性事件
EventBus.getDefault().postSticjy(new StickyEvent("我是粘性事件"));
